...
def embed_binary():
    ...
    binary = os.path.join(home, "dev/go-projects/notify/noti")
    # pdfFile = input("Path to the PDF to be embed => ")
    pdfFile = os.path.join(home, "dev/dttw-zine01.pdf")
...
def embed_link():
    ...
    pdf.cell(200, 10, txt="Booby-Trap", ln=1, align="C")
    pdf.cell(200, 10, txt=link, border=1, ln=2, align="C")
    pdf.output("BoobyLinked.pdf")


def parse_arguments():
    parser = argparse.ArgumentParser()
    parser.add_argument("--link", "-l", type=str, required=False)
    parser.add_argument("--binary", "-b", type=str, required=False)
    parser.add_argument("--help", "-h", type=str, required=False)
