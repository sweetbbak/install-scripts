...
def install():
    ...
    locations = {
        ...
        "chrome-beta": os.path.join(
            home_path, ".config", "google-chrome-beta", "NativeMessagingHosts"
        ),
        "chrome-unstable": os.path.join(
            home_path, ".config", "google-chrome-unstable", "NativeMessagingHosts"
        ),
    ...
    for browser, location in locations.items():
        if os.path.exists(os.path.dirname(location)):
            ...
            with open(os.path.join(location, filename), "w") as file:
                file.write(
                    json.dumps(
                        browser_manifest, indent=2, separators=(",", ": "), sort_keys=True
                    ).replace("  ", "\t")
                    + "\n"
                )
...
def listen():
    ...
    if receivedMessage == "ping":
        sendMessage({"version": VERSION, "file": os.path.realpath(__file__)})
